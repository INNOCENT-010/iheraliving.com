...
export default function EnquiriesPage() {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ backgroundColor: '#0d0d0d' }}>
      ...
      {/* ── Left: List ── */}
      <div
        className="w-full lg:w-[420px] shrink-0 flex flex-col border-r overflow-hidden"
      ...
      >
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              {newCount > 0 && (
                <p className="font-body text-[9px] tracking-widest uppercase mt-0.5" style={{ color: '#b8924a' }}>
                  {newCount} new
                </p>
              )}
            </div>
          ...
          {/* Filter tabs */}
          <div className="flex gap-0 border-b" style={{ borderColor: 'rgba(184,146,74,0.1)' }}>
            {STATUS_FILTERS.map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className="font-body text-[8px] tracking-widest uppercase px-3 py-2 transition-colors capitalize border-b-2 -mb-px"
                style={{
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <div className="p-6">
              <p className="font-body text-xs" style={{ color: 'rgba(245,240,232,0.3)' }}>Loading...</p>
            </div>
          ) : enquiries.length === 0 ? (
            <div className="p-6 text-center">
              <p className="font-body text-sm" style={{ color: 'rgba(245,240,232,0.2)' }}>No enquiries.</p>
            </div>
            ...
            >
              <div className="flex items-start justify-between gap-2 mb-1">
                <p className="font-body text-sm truncate" style={{ color: enquiry.status === 'new' ? '#f5f0e8' : 'rgba(245,240,232,0.7)' }}>
                  {enquiry.name}
                </p>
                <div className="flex items-center gap-1.5 shrink-0">
                  {enquiry.status === 'new' && (
                    <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: '#b8924a' }} />
                  )}
      ...
      <div className="hidden lg:flex flex-col flex-1 overflow-hidden">
        {selected ? (
          <div className="flex flex-col h-full">
            ...
            >
              <div>
                <p className="font-display text-xl mb-1" style={{ color: '#f5f0e8' }}>{selected.name}</p>
                <a
              ...
              <div className="flex items-center gap-2">
                {selected.status !== 'replied' && (
                  <button
                    ...
                    className="flex items-center gap-2 font-body text-[9px] tracking-widest uppercase px-4 py-2 transition-colors"
            ...
            >
              <div>
                <span className="font-body text-[8px] tracking-widest uppercase block" style={{ color: 'rgba(245,240,232,0.3)' }}>
                  Received
                </span>
            ...
            >
              <a
                ...
                className="inline-flex items-center gap-3 font-body text-[10px] tracking-widest uppercase px-8 py-3 transition-colors"
